...
static int subsystem_restart_thread(void *data)
{
	...
	int retry_cnt = 0;//FEATURE_SAMSUNG_SILENT_RESET, waro.park. power up fail 발생하는 문제 수정
...
#if 0//FEATURE_SAMSUNG_SILENT_RESET, waro.park. power up fail 발생하는 문제 수정
		if (restart_list[i]->powerup(subsys->name) < 0)
			panic("%s: Failed to powerup %s!", __func__,
				restart_list[i]->name);
#else
    retry_cnt = 0;
		while ((restart_list[i]->powerup(subsys->name) < 0) && (retry_cnt < 5))
		{
			retry_cnt++;
			pr_info("%s: Failed to powerup %s!", __func__, restart_list[i]->name);
			msleep(10);
		}
		if(retry_cnt >= 5)
			panic("%s: Failed to powerup %s!", __func__,
