...
#define SDIO_BOOTLOADER_CARD_INDEX 1
 ...
 || defined (CONFIG_USA_MODEL_SGH_I727) || defined (CONFIG_USA_MODEL_SGH_T989)|| defined (CONFIG_USA_MODEL_SGH_T769) || defined (CONFIG_USA_MODEL_SGH_I577)
...
static uint32_t comm_error;    //temporary for silent reset fail,by kyungjin.moon
...
int sdio_write(struct sdio_channel *ch, const void *data, int len)
{
	...
	if (ch->signature != SDIO_AL_SIGNATURE) {
		pr_err(MODULE_NAME ":%s: Invalid signature 0x%x\n",  __func__,
			ch->signature);
        //temporary for silent reset fail,by kyungjin.moon
		sdio_al_dev = ch->sdio_al_dev;
