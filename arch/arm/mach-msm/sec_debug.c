...
#define MAX_DEBUG_TIMER_INDEX 	1024

typedef struct
{
	unsigned long mTimerAddr;
	unsigned long mCallerAddr;
	pid_t mPid;
}secTimerList;

static secTimerList g_SecTimerList[MAX_DEBUG_TIMER_INDEX];
static int gSecDebugTimerIndex = 0;

void sec_debug_Timer(void *timer, unsigned long pCallerAddr)
{
	gSecDebugTimerIndex = (gSecDebugTimerIndex+1) & DEBUG_INDEX_MASK;
	
	g_SecTimerList[gSecDebugTimerIndex].mTimerAddr	= (unsigned long)timer;
	g_SecTimerList[gSecDebugTimerIndex].mCallerAddr = pCallerAddr;
	g_SecTimerList[gSecDebugTimerIndex].mPid = current->pid;

}
EXPORT_SYMBOL(sec_debug_Timer);
