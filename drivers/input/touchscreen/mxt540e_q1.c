...
#include "mXT540e__APP_V1_2_F2_.h"
...
static void get_user_data(struct mxt540e_data *data)
{
	u16 t38_address = 0;
	u16 size;
	// u8 value;
	...
	read_mem(data, t38_address + 0, 1, &mxt540e_dbg.config_init_disable);
	read_mem(data, t38_address + 1, 1, &mxt540e_dbg.ta_probe_disable);
	read_mem(data, t38_address + 2, 1, &mxt540e_dbg.t55_ctrl_batt);
	read_mem(data, t38_address + 3, 1, &mxt540e_dbg.t55_thr_batt);
	read_mem(data, t38_address + 4, 1, &mxt540e_dbg.t57_thr_batt);
	read_mem(data, t38_address + 5, 1, &mxt540e_dbg.t55_ctrl_ta);
	read_mem(data, t38_address + 6, 1, &mxt540e_dbg.t55_thr_ta);
	read_mem(data, t38_address + 7, 1, &mxt540e_dbg.t57_thr_ta);
}
...
static void mxt540e_ta_probe(int ta_status)
{
	...
	if(mxt540e_dbg.ta_probe_disable == 1)
	{
		printk("[TSP] T38 mxt540e_ta_probe Disable: %d", mxt540e_dbg.ta_probe_disable);
		return;
	}
#endif
	...
	if (ta_status) {
		...
		get_object_info(data, ADAPTIVE_T55, &size, &obj_address);
		value = 0;
		error |= write_mem(data, obj_address, 1, &value);
#if 0
		error |= write_mem(data, obj_address+0, 1, &mxt540e_dbg.t55_ctrl_ta);
		error |= write_mem(data, obj_address+1, 1, &mxt540e_dbg.t55_thr_ta);
		
		get_object_info(data, SPT_GENERICDATA_T57, &size, &obj_address);		
		error |= write_mem(data, obj_address+1, 1, &mxt540e_dbg.t57_thr_ta);
#endif
	} else {
		...
		error |= write_mem(data, obj_address, 1, &value);
#if 0
		error |= write_mem(data, obj_address+0, 1, &mxt540e_dbg.t55_ctrl_batt);
		error |= write_mem(data, obj_address+1, 1, &mxt540e_dbg.t55_thr_batt);
		
		get_object_info(data, SPT_GENERICDATA_T57, &size, &obj_address);		
		error |= write_mem(data, obj_address+1, 1, &mxt540e_dbg.t57_thr_batt);
#endif
	}
...
static int mxt540e_load_fw(struct device *dev, const char *fn)
{
    ...
    fw->size = ARRAY_SIZE(mXT540e__APP_V1_2_F2);
    fw->data = mXT540e__APP_V1_2_F2;
...
{
	pr_info("mxt_touchtype_show :: Latest firmware version is %d\n", firmware_latest);
	return sprintf(buf, "ATMEL %#02x\n", firmware_latest);
}
#else 
static ssize_t mxt_touchtype_show(struct device *dev, struct device_attribute *attr, char *buf)
{
