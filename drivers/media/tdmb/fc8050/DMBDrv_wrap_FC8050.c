...
unsigned char DMBDrv_init(void)
{
	...
	BBM_FIC_CALLBACK_REGISTER(0, TDMBDrv_FIC_CALLBACK);
	BBM_MSC_CALLBACK_REGISTER(0, TDMBDrv_MSC_CALLBACK);
...
unsigned char DMBDrv_ScanCh(unsigned long ulFrequency)
{
	...
	if(BBM_TUNER_SET_FREQ(NULL, ulFrequency)) {
        DPRINTK("%s : BBM_TUNER_SET_FREQ error \n", __func__);	
		BBM_WORD_WRITE(NULL, BBM_BUF_INT, 0x00ff);
		return TDMB_FAIL;
	} 

	if(BBM_SCAN_STATUS(NULL)) {
        DPRINTK("%s : BBM_SCAN_STATUS error \n", __func__);	
		BBM_WORD_WRITE(NULL, BBM_BUF_INT, 0x00ff);
		return TDMB_FAIL;
	}
	...
	if(esb->flag != 99) {
        DPRINTK("%s : esb->flag != 99 \n", __func__);	
		FIC_DEC_SubChInfoClean();
		return TDMB_FAIL;
	}

	if(strlen(esb->label) <= 0) {
        DPRINTK("%s : strlen(esb->label) <= 0 \n", __func__);		
		FIC_DEC_SubChInfoClean();
		return TDMB_FAIL;
	}
