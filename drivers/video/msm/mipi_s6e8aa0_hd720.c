...
static void update_LCD_SEQ_by_id(struct lcd_setting *destLCD)
{
	...
	pBuffer[0] = 0;
	sprintf( pBuffer +strlen(pBuffer), "%s :", __func__ );
	...
	switch( destLCD->factory_id_colorTemp )
	{
		...
		sprintf( pBuffer +strlen(pBuffer), " 7500K" );
		updateCnt++;
		break;
		...
		sprintf( pBuffer +strlen(pBuffer), " 8500K" );
	...
	for( i = 0; i < ARRAY_SIZE( s6e8aa0_display_on_cmds ); i++ )
	{
		if( isPANEL_COND_SET(s6e8aa0_display_on_cmds[i].payload))
		{
			...
			updateCnt++;
			sprintf( pBuffer +strlen(pBuffer), " pc%d", i );
		}
		...
		if( is_ETC_COND_0xD9_SET(s6e8aa0_display_on_cmds[i].payload))
		{
			...
			updateCnt++;
			sprintf( pBuffer +strlen(pBuffer), " fc%d", i );
		}
	...
	for( i = 0; i < ARRAY_SIZE( s6e8aa0_display_on_before_gamma_cmds ); i++ )
	{
		if( isPANEL_COND_SET(s6e8aa0_display_on_before_gamma_cmds[i].payload))
		{
			...
			updateCnt++;
			sprintf( pBuffer +strlen(pBuffer), " pb%d", i );
		}
		...
		if( is_ETC_COND_0xD9_SET(s6e8aa0_display_on_before_gamma_cmds[i].payload))
		{
			...
			updateCnt++;
			sprintf( pBuffer +strlen(pBuffer), " fb%d", i );
		}
	}
	...
	if( destLCD->isSmartDimming && s6e8aa0_lcd.isSmartDimming_loaded )
	{
		...
		{
			case 1:
				pNew_GAMMA_SmartDimming_VALUE_SET_300cd = (unsigned char*) GAMMA_SmartDimming_VALUE_SET_AID1_300cd;
				sprintf( pBuffer +strlen(pBuffer), " iAID1" );
			break;
			case 2:
				pNew_GAMMA_SmartDimming_VALUE_SET_300cd = (unsigned char*) GAMMA_SmartDimming_VALUE_SET_AID2_300cd;
				sprintf( pBuffer +strlen(pBuffer), " iAID2" );
			break;
			case 3:
				pNew_GAMMA_SmartDimming_VALUE_SET_300cd = (unsigned char*) GAMMA_SmartDimming_VALUE_SET_AID3_300cd;
				sprintf( pBuffer +strlen(pBuffer), " iAID3" );
			break;
			default:
				pNew_GAMMA_SmartDimming_VALUE_SET_300cd = (unsigned char*) GAMMA_SmartDimming_VALUE_SET_300cd;
	...
	switch( s6e8aa0_lcd.factory_id_dcdc )
	{
		case lcd_id_dcdc_STOD03A:
			...
			sprintf( pBuffer +strlen(pBuffer), " ed03" );
		break;
		case lcd_id_dcdc_STOD13A:
			s6e8aa0_lcd.lcd_elvss_table = lcd_elvss_dcdc13_table;
			sprintf( pBuffer +strlen(pBuffer), " ed13" );
		break;

		case lcd_id_dcdc_F:
			s6e8aa0_lcd.lcd_elvss_table = lcd_elvss_dcdc03_table;
			sprintf( pBuffer +strlen(pBuffer), " ed?03" );
		default:
		break;
	}
...
//	elvss_Minus10 = (srcBuffer[1] == 0x4E);
	...
	switch( checkValue )
	{
		case 2:
			...
			cntBuf += sprintf( pBuffer +cntBuf, " STOD13A" );
		break;
		case 3:
			destLCD->factory_id_dcdc = lcd_id_dcdc_STOD03A;
			cntBuf += sprintf( pBuffer +cntBuf, " STOD03A" );
		break;
		case 4:
			destLCD->factory_id_dcdc = lcd_id_dcdc_F;
			cntBuf += sprintf( pBuffer +cntBuf, " F" );
		break;
		default:
			destLCD->factory_id_dcdc = lcd_id_unknown;
			cntBuf += sprintf( pBuffer +cntBuf, " dcdc?" );
		break;
	}
	cntBuf += sprintf( pBuffer +cntBuf, "(%X)", checkValue );
	...
	switch( checkValue )
	{
		case 1:
			...
			cntBuf += sprintf( pBuffer +cntBuf, " EVT1" );
		break;
		case 2:
			destLCD->factory_id_EVT = lcd_id_EVT2;
			cntBuf += sprintf( pBuffer +cntBuf, " EVT2" );
		break;
		default:
			destLCD->factory_id_EVT = lcd_id_unknown;
			cntBuf += sprintf( pBuffer +cntBuf, " EVT?" );
		break;
	}
	cntBuf += sprintf( pBuffer +cntBuf, "(%X)", checkValue );

	cntBuf += sprintf( pBuffer +cntBuf, " id3=%X", srcBuffer[2]);
	...
	if( checkValue )
	{
		...
		cntBuf += sprintf( pBuffer +cntBuf, " AID=%d", destLCD->AID );

		destLCD->cellPulse = (checkValue & 0x1F);
		cntBuf += sprintf( pBuffer +cntBuf, " cp=%d", destLCD->cellPulse );
