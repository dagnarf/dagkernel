...
int mdp4_overlay_set(struct fb_info *info, struct mdp_overlay *req)
{
	...
	{		
		if(req->dst_rect.w == 720 && req->dst_rect.h == 1280)
		{
			if((req->src_rect.w >= req->src_rect.h) && (req->src_rect.h > 720))
				mdp4_dsi_video_blt_start(mfd);

		}
		else if(req->dst_rect.w == 1280 && req->dst_rect.h == 720)
		{
			if((req->src_rect.w <= req->src_rect.h) && (req->src_rect.w > 1280) )
				mdp4_dsi_video_blt_start(mfd);
		}
	}	
